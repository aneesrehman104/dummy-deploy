...
export const backEndURLWithoutAuthDefaultHeader = axios.create({
    baseURL: `${process.env.NEXT_PUBLIC_API_URL}`,
    withCredentials: true,
});
...
const toODataQuery = (params: ODataParams): string => {
    ...
    //if param.filter is not null or not '' or not undefined, then add it to the odataParamsArr
    ...
    if (params.count !== undefined)
        odataParamsArr.push(`$count=${params.count}`);
...
export const getODataWithParams = async (url: string, params: ODataParams) => {
    try {
        ...
        const result = await backEndURLWithoutAuthDefaultHeader.get(fullUrl);
